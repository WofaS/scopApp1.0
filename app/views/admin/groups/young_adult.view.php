...
<div class="pagetitle rounded p-0 border-bottom">
   <nav>   
        <label class="fw-bolder badge text-primary"><?=strtoupper('District Young-Adults Overview')?></label>
        ...
        <ul class="breadcrumb my-0 py-0">
          ...
          <?php
           ...
           foreach ($data['row'] as $row){
                 ...
                 if(!empty($row->category_id)){
                    ...
                    if($row->role_name === 'member' OR $row->role_name === 'elder' OR $row->role_name === 'deacon' OR $row->role_name === 'deaconess' OR $row->role_name === 'sofomaame' OR $row->role_name === 'pastor'){
                    ...
                    if($age > 19 AND $age < 36)
                      ...
                      if($age > 19 AND $age < 36 AND $row->gender ==='male')
                      $numMM += 1;

                      $totalMM =$numMM;

                      if($age > 19 AND $age < 36 AND $row->gender ==='female')
                      $numMF += 1;
                    ...
                    if($age > 19 AND $age < 36 AND $row->role_name === 'visitor')
                      $numV += 1;
                ...
                ?>
              <i class="bi bi-people-fill"></i></div>Total Young-Adults <b class="mx-2 mb-0 text-danger"><?=$totalT ?? 0?></b>
          ...
          <li class="card-title mx-auto d-flex">
            <div class="card-icon rounded-circle text-primary fw-bolder mx-auto" ><i class="bx bx-male"></i></div>Young-Adult Members <b class="mx-3 text-danger"><?=$totalM ? : 0?></b>
            <div class="filter col float-end">
            <a class="icon" href="#" data-bs-toggle="dropdown"><i class="bi bi-three-dots"></i></a>
            <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
            <li class="dropdown-header text-start">
          ...
          <li class="card-title mx-auto d-flex">
            <div class="card-icon rounded-circle text-primary fw-bolder mx-auto" ><i class="bx bx-female"></i></div>Young-Adult Visitors <b class="mx-3 text-danger"><?=$totalV ?? 0?></b>
          </li>
        </ul>
      </nav>
    </div><!-- End Page Title -->
      ...
      <!-- Profile Edit Form -->
        <div class="card-body">
            ...
            <div class="section-header d-flex justify-content-between align-items-center mb-2">
              <h5 class="text-muted"><?=strtoupper(esc('Sampa District - Teens'))?></h5>
              <p class="col-6">Note: <span class="fst-italic">These are youth members from age of 20 to 35 years. It comprises officers, non-officers and 'visitors'.</span></p>
            </div>      
        </div>      
              ...
              <thead>
                <tr style="background: lightgray;">
                  ...
                  <th scope="col" data-sortable="" >ACTION
                    <a href="<?=ROOT?>/admin/excel/print_Young_Adults"><button class="btn btn-success btn-sm"><i class="bi bi-file-earmark-excel p-0"></i>  Download Excel</button></a>
                  </th>
                </tr>
              </thead>
              <tbody>
