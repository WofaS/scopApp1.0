<?php
  ...
  $queryElder = "select count(id) as num from members where role = '5' AND category_id = 'Elshadai'";
  $resElder = query_row($queryElder);

  $queryDeaconess = "select count(id) as num from members where role = '6' AND category_id = 'Elshadai'";
  $resDeaconess = query_row($queryDeaconess);

  $queryDeacon = "select count(id) as num from members where role = '7' AND category_id = 'Elshadai'";
...
<div class="pagetitle rounded p-0 border-bottom">
   <nav>   
        <label class="fw-bolder badge text-primary"><?=strtoupper('Elshadai Officers Overview')?></label>
...
<?php if(!empty($data['row'])):?>
    <div class="section-header d-flex justify-content-between align-items-center mb-5">
      <h2 class="text-muted border-bottom"><?=strtoupper('Elshadai ASSEMBLY Officers')?></h2>
    </div>
  <div class="row g-5">
