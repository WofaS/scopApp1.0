<?php
  ...
  $queryElder = "select count(id) as num from members where role = '5' AND category_id = 'Royals'";
  $resElder = query_row($queryElder);

  $queryDeaconess = "select count(id) as num from members where role = '6' AND category_id = 'Royals'";
  $resDeaconess = query_row($queryDeaconess);

  $queryDeacon = "select count(id) as num from members where role = '7' AND category_id = 'Royals'";
...
<div class="pagetitle rounded p-0 border-bottom">
   <nav>   
        <label class="fw-bolder badge text-primary"><?=strtoupper('Royals Officers Overview')?></label>
...
<?php if(!empty($data['row'])):?>
    <div class="section-header d-flex justify-content-between align-items-center mb-5">
      <h2 class="text-muted border-bottom"><?=strtoupper('Royals ASSEMBLY Officers')?></h2>
    </div>
  <div class="row g-5">
  ...
  <?php if(!empty($row->category_id) && $row->category_id === 'Royals'):?>
