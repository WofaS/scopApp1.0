<?php
  ...
  $query = "select count(id) as num from members where category_id = 'Latter Rain'";
  $resLocal = query_row($query);

  $queryMale = "select count(id) as num from members where gender = 'male' AND category_id = 'Latter Rain'";
  $resLocalMale = query_row($queryMale);
  
  $queryMaleChild = "select count(id) as num from members where role = '8' AND gender = 'male' AND category_id = 'Latter Rain'";
  $resLocalMaleChild = query_row($queryMaleChild);

  $resLocalMaleAdult = $resLocalMale['num'] - $resLocalMaleChild['num'];

  $queryFemale = "select count(id) as num from members where gender = 'female' AND category_id = 'Latter Rain'";
  $resLocalFemale = query_row($queryFemale);

  $queryFemaleChild = "select count(id) as num from members where role = '8' AND gender = 'female' AND category_id = 'Latter Rain'";
  $resLocalFemaleChild = query_row($queryFemaleChild);

  $resLocalFemaleAdult = $resLocalFemale['num'] - $resLocalFemaleChild['num'];

  $query1 = "select count(id) as num from members where role = '1' AND category_id = 'Latter Rain'";
  $resLocal1 = query_row($query1);

  //admin count
  $query2 = "select count(id) as num from members where role = '2' AND category_id = 'Latter Rain'";
  $resLocal2 = query_row($query2);

  //members count
  $query3 = "select count(id) as num from members where role = '4' AND category_id = 'Latter Rain'";
  $resLocal3 = query_row($query3);

  //elders count
  $query4 = "select count(id) as num from members where role = '5' AND category_id = 'Latter Rain'";
  $resLocal4 = query_row($query4);

  //dcnss count
  $query5 = "select count(id) as num from members where role = '6' AND category_id = 'Latter Rain'";
  $resLocal5 = query_row($query5);

  //dcn count
  $query6 = "select count(id) as num from members where role = '7' AND category_id = 'Latter Rain'";
  $resLocal6 = query_row($query6);

  //child count
  $query7 = "select count(id) as num from members where role = '8' AND category_id = 'Latter Rain'";
  $resLocal7 = query_row($query7);
  
  //visitor count
  $query8 = "select count(id) as num from members where role = '9' AND category_id = 'Latter Rain'";
...
<div class="pagetitle rounded p-0 border-bottom">
   <nav>   
        <label class="fw-bolder badge text-primary"><?=strtoupper('Latter Rain Membership Overview')?></label>
      ...
      <!-- Profile Edit Form -->
        <div class="card-body">
            ...
            <div class="section-header d-flex justify-content-between align-items-center mb-2">
              <h2 class="text-muted"><?=strtoupper(esc('Latter Rain ASSEMBLY'))?></h2>
            </div>      
            <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
              ...
              <thead>
                <tr class="bg-secondary text-light">
                  ...
                  <th scope="col" data-sortable="" >
                    <a href="<?=ROOT?>/admin/excel/print_latter_rain">
              ...
              <tbody>
                ...
                <?php if(!empty($row->category_id) && $row->category_id === 'Latter Rain'): $id +=1;?>
