...
<?php $this->view('includes/navigation',$data)?>
...
<div class="container d-flex">
    <div class="col-lg-6 col-md-10 col-12 pt-4  mx-auto align-items-center shadow rounded bg-light mb-5">
      <div class=" px-3 py-2 rounded text-light bg-secondary" style="">
        <div class="row d-flex mx-0 px-0">
          <div class="col-8 my-auto">
            <center class="border-bottom fw-bolder text-center mx-auto text-info">About <?=ucfirst($row->firstname)?> <?=ucfirst($row->lastname)?></center>
            <p class="small fst-italic"><?= ucfirst($row->firstname ).' '. ucfirst($row->lastname)?> <?='('.ucfirst($row->role_name ? :'No role').') of Sampa English Assembly serves in the local as '?> <?=$row->position_id ? :$row->role_name?><?='. '.ucfirst($row->firstname). ' was born on the '.get_date($row->dob).' to '.$row->mother_name.' (Mother) and '. $row->father_name." (Father). Currently ".$row->firstname."'s occupation is ". $row->job.' and stays at '.$row->residence.' in Sampa-Jaman North District of the Bono Region, Ghana.'?></p>
          ...
          <div class="col-4 float-end my-auto">
            ...
            <?php endif;?>
          <div class="badge" style="background: darkblue;"><?=$age?></div>
        </div> 
        <br>
       ...
       <div class="table table-hover bg-light text-dark mx-0 px-3 rounded mb-3 mt-2" id="profile-overview" style="font-size: 16px;">            
        ...
        <!-- Other Details -->
            <div class="row  col-lg-6 mx-0 p-2 shadow ">
              ...
              <tr class="row">
                <strong class="col-4 col-lg-4 col-md-4 col-sm-5 label">Address: </strong>
                <div class="col-8 col-lg-8 col-md-8 col-sm-7"><?=esc($row->address ? :'Not available')?></div>
              </tr> 
